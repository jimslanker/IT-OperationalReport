-----------------------------------------------------------------
-- Filename: cw.UserTicketsByMonth.sql
-- Author: Ryan Joaquim
...
/****** Object:  View [CW].[UserTicketsByMonth]    Script Date: 6/8/2022 11:13:03 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER view [CW].[UserTicketsByMonth] as
SELECT  OwnedBy
		,OwnedByTeam
		,Teams
		,isnull(Aug,0) Aug
...
FROM (
	SELECT inc.OwnedBy
		,inc.OwnedByTeam
		,FORMAT(CreatedDateTime, 'MMM', 'en-US') Mon
		,count(*) coun
...
when 'Cherwell Platform Admin' then 'Corporate'
		...
		,Total
	FROM [CW].[STAGE_INCIDENTS] inc left join (select OwnedBy,OwnedByTeam,count(*) total from [CW].[STAGE_INCIDENTS] WHERE CreatedDateTime >= '8/1/21' group by OwnedBy,OwnedByTeam ) inc2 on inc.OwnedBy = inc2.OwnedBy and inc.OwnedByTeam = inc2.OwnedByTeam
	WHERE CreatedDateTime >= '8/1/21'
	GROUP BY inc.OwnedByTeam
		,inc.ownedby
		,FORMAT(CreatedDateTime, 'MMM', 'en-US')
		,total
	) t
