-----------------------------------------------------------------
-- Filename: cw.vwItOperationReportChangeReqs.sql
-- Author: Ryan Joaquim
...
/****** Object:  View [CW].[vwItOperationReportChangeReqs]    Script Date: 6/8/2022 11:19:26 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER view [CW].[vwItOperationReportChangeReqs] as
SELECT cr.[ChangeID]
      ,cr.[ChangeType]
      ,cr.[ChangeOwner]
      ,cr.[OwnedByTeam]
      ,cr.[RequestedBy]
	  ,cr.[CreatedDateTime]
	  ,cast(cr.[CreatedDateTime] as date) CreatedDate
	  ,cr.[LastModifiedDateTime]
      ,cr.[Status]
      ,cr.[Title]
      ,cr.[Description]
	  ,cr.[CreatedBy]
      ,cr.[Division]
      ,cr.[Location]
      ,case when cr.[PassedTestingDateTime] <> '1900-01-01 00:00:00.000' then cast(cr.[PassedTestingDateTime] as date) else null end PassedTestingDate
      ,cr.[PassedTestingCertificationUser]
	  ,datediff(d,cr.LastModifiedDateTime,getdate()) DaysSinceUpdate
	  ,case when cr.ChangeID is not null  then 'https://corp-hdeskprod.cherwellondemand.com/CherwellClient/Access/changerequest/' + cast(cr.ChangeID as varchar(16)) else null end as ChangeLink
	  ,ap.[ApprovalID]
      ,ap.[Status] ApproverStatus
      ,ap.[ActualApproverName]
      ,ap.[ObjectApprovalType]
      ,ap.[ApproverComment]
      ,ap.[WhenApprovedDenied]
	  ,case when ap.Status in ('Approved','Denied') then cast(ap.[WhenApprovedDenied] as date) else null end WhenApprovedDeniedDate
...
when 'METL-New Employee' then 'SD'
  ...
  FROM [CW].[STAGE_CHANGE_REQUESTS] cr left join [CW].[vwApprovalsActualParents] ap on cr.ChangeID = ap.ActualParentID
  where ap.ActualParentType = 'change request'
GO
